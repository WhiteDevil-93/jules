...
| **Delete Session**       | Remove a session from the local cache to declutter your session list.                                                                                                                                              | `jules-extension.deleteSession`     |
| **Clear Cache**          | Clear all cached data, including session history and activities, to start fresh.                                                                                                                                   | `jules-extension.clearCache`        |
...
This extension requires authentication to access your Jules account and associated GitHub repositories.
...
4.  Once authorized, you will be automatically signed in.

### Method 2: GitHub Personal Access Token (PAT) (Deprecated) ⚠️

> **Note:** PAT support is deprecated and will be removed in a future version. Please migrate to the OAuth sign-in method for better security and convenience.

If you need to use a PAT, you can configure it with the `> Jules: Set GitHub Token (for PR Status)` command. However, this method is no longer recommended.
...
- `jules-extension.customPrompt`: A custom prompt to automatically prepend to every message sent to Jules, acting as a persistent instruction.
...
- `jules.githubPat`: **[DEPRECATED]** Use the `Jules: Sign in to GitHub` command instead. PAT support will be removed in a future version.
...
-   `Jules: Set GitHub Token (for PR Status)` - **[DEPRECATED]** Set a GitHub PAT for checking PR status.
